...
def show_admin_panel(user_email):
    is_admin = user_email.lower() in [e.lower() for e in st.secrets.get("admins", [])]
    if not is_admin:
        return
    ...
    if st.session_state.get("view") == "admin":
        st.title("📊 Admin Panel - View Submissions")
        responses = load_responses()  # must exist in working.py
        if not responses.empty:
            teachers = ["All"] + sorted(responses["Teacher"].unique().tolist())
            choice = st.selectbox("Filter by Teacher", teachers)

            if choice != "All":
                filtered = responses[responses["Teacher"] == choice]
            else:
                filtered = responses
